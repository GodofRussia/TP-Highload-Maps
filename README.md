...
Кликхаус гибрид внутри и работает из коробки, как и монга (в ней нужно просто добавить хеш функцию на номер шарда).
...
Нужно по реляционным базам делать минимум 2 реплики Master-Slave на один инстанс СУБД:
...
Индексируем Points:
    CREATE INDEX point_index ON Tiles (x, y, z);

## 7. Алгоритмы

Для построения маршрутов[^1] мы используем алгоритм A*, это тот же алгоритм Дейкстры, но при этом он использует эвристику, то есть использует некие параметры для предположения о дальнейшем пути. Маршруты строятся для человека, кешируются для него, поскольку, например, хранить в кеше каждый маршрут невыгодно (большой кеш мисс).

https://bigdataschool.ru/blog/cypher-queries-to-find-shortest-path-in-neo4j.html
