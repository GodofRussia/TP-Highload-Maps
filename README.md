...
Скрипт в ClickHouse переписывает данные из hadoop (после Spark) про сегменты и их средние скорости в табличке ExternalSegment.
...
| Address | после индексации (около 0) | 926 RPS |
| RoadSegmentNode | 0 | 3704 RPS |
| ExternalSegment | 50 000 | 3704 RPS |
| SearchAddress |  | 926 RPS |
| SearchTag |  | 926 RPS |
...
***Cassandra*** - NoSQL СУБД для одновременного чтения и записи. Используем для хранения пользовательских данных.
...
`HDFS` на s3 для тайлов. Для s3 отдельно дать инфу, может в том же хадупе будет обработка через отдельный бекенд. Или всё-таки разные хадупы под разные задачи.

### 6.2 Шардирование и резервирование СУБД

Используем два вида шардинга - классические шарды и партицирование в реляцонной субд. Можно брать номер шарда, как хеш от айди (ограничиваем количеством шардов). При этом лучше делать прешардирование. Создаем заранее большое кол-во шардов. При этом есть виртуальные шарды - они физически мапятся на одной машине. Виртуальные шарды - отдельные инстансы. Облегчает удаление данных при дальнейшем масштабировании.
...
|--|--|--|
| id | PK формируется на клиенте и однозначно определяет png ресурс и строку в таблице | 8 байт |
| evenData | Bsonы, внутри которых хранится метаинформация про дорожные события | 1 Кбайт |
| language | Строка формата 'ru_Ru' или 'en_En', определяет язык отображения | 5-10 байт |
...
|--|--|--|
| Golang | Backend | Простота языка, удобные тулзы из коробки, высокая утилизация CPU |
| Python | Backend и скрипты | Язык для написания скриптов Spark и Elastic |
