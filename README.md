...
- [Репозиторий с проектированием курсовой работы по хайлоаду на тему Яндекс.Карты.](#репозиторий-с-проектированием-курсовой-работы-по-хайлоаду-на-тему-яндекскарты)
  ...
  - [5. Логическая схема базы данных](#5-логическая-база)
  - [6. Физическая схема базы данных](#6-физическая-база)
...
**Тайлы (Tile):**
Здесь и дальше все src картинок в виде url (uri). Такие картинки хранятся на S3 HDFS. Информация по тайлам хранится в реляционной СУБД.

**Точка на карте (Point) и Адрес и Сессия:**
Тоже таблички в реляционной СУБД, используем PostGis расширение для функции геокодинга.

**Дорожный граф (RoadSegmentNode):**
Лежит в графовой СУБД, поле средней скорости переподсчитывается раз в какое-то время, на функциях API СУБД считаем цену по сгементу и строим маршруты.

Скрипт в ClickHouse переписывает данные из hadoop (после MapReduce) про сегменты и их средние скорости в табличке ExternalSegment.

**Маршруты и пользовательские данные:**

Хранятся в Mongo, поскольку имеют сложную структуру, похожую на документ, записываются не так часто, как читаются.

**Отдельно про кеши:**

У таблицы тайлов есть кеширование запросов (подробнее в физ схеме). Маршруты кешируются на пользователя.

![Alt text](telegram-cloud-photo-size-2-5375237891652964146-y.jpg)

## 6. Физическая схема базы данных

### 6.1 Денормализация

Так как схема сразу достаточно нормализованна, то действий не так много:

- Нужно в MongoDB хранить адреса и запросы, поэтому мы обновляем документы пользователей при необходимости (изменение с пользовательских действий).

### 6.2 Базы данных

Реляционная база - ***Postgres***. Таблицы - Session, Address, Tiles.

Графовая СУБД - ***Neo4j***.

***ClickHouse*** - для быстрой записи и колоночного чтения.
...
***Агрегация событий на дороге:***

Через кафку делаем запись событий (то есть собирающихся треков GPS водителей) в `Hadoop`, потом используя алгоритмы `Spark` (применяем код) мы делаем MapReduce данных, а потом через внутреннюю кафку записываем их в `ClickHouse`. Оттуда их читает бекенд для дорожных событий.
...
`HDFS` на s3 для тайлов.

### 6.3 Шардирование и резервирование СУБД

Используем два вида шардинга - классические шарды и партицирование в реляцонной. Можно брать номер шарда, как хеш от айди (ограничиваем количеством шардов). При это лучше делать прешардирование. Создаем заранее большое кол-во шардов. При этом есть виртуальные шарды - они физически мапятся на одной машине. Виртуальные шарды - отдельные инстансы или таблички в бд. Облегчает удаление данных при дальнейшем масштабировании.

Кликхаус гибрид шардинга внутри и работает из коробки, как и монга (в ней нужно просто добавить хеш функцию на номер шарда).

В MongoDB таблицу маршутов для пользователей можно шардировать еще и по реестру пользователь помимо хеш функции (для большего баланса).
...
Индексируем Tiles:
    ...
    CREATE INDEX tiles_index ON Tiles (x, y, z);

Индексируем Points:

    CREATE INDEX point_coordinates_index ON Tiles (coordinates);
    CREATE INDEX point_address_index ON Tiles (address);

## 7. Алгоритмы

### 7.1 Построение маршрутов

Для построения маршрутов[^1] мы используем алгоритм A*[^23], это тот же алгоритм Дейкстры, но при этом он использует эвристику, то есть использует некие параметры для предположения о дальнейшем пути. Маршруты строятся для человека, кешируются для него, поскольку, например, хранить в кеше каждый маршрут невыгодно (большой кеш мисс).

Алгоритм внутри работает следующим образом:

- Из графовой базы забираем по внутреннему API маршрут (сегменты)[^24].
- Читаем из кликхауса текущие данные по колонке external_segment_id.
...
- Треки GPS (агрегированные оттуда данные) поступают на распределенные ноды Hadoop раз в какое-то время
- Оттуда после MapReduce проходят через Kafka в ClickHouse, мапясь с сегментами. Тут нужна поддержка програмирования поверх данных, поэтому задействуем Spark.
- Кроной обовляем среднюю скорость на нодах дорожного графа
- Когда приходит запрос на рассчёт расстрояния с учётом пробок, мы берем актуальные данные после получения сегментов из кликхауса.
